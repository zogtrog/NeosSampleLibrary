...
namespace Mongo4Neos
{
    ...
    public class MongoFind : LogixNode
    {
        public const string no_id = "ID0";
        private Task<QArray> WebRequestTask = null;
        ...
        public readonly Input<ulong> Position;
          public readonly Output<Guid> CurrentDocID;
        ...
        private QArray GetBookMarks(QArray RQ)
        {
            string[] SA;
            int sac,x;
            ulong tc;
            ...
            try
            {
                ...
                sac = SA.Length;
                tc = 0;
                RQ.docs = new Guid[sac];
                for(x=0;x<sac;x++)
                {
                    if(Guid.TryParse(SA[x],out RQ.docs[tc]))
                    {
                        tc++;
                    }
        ...
        protected override void OnCommonUpdate()
        {
            ...
            else
            {
                if(pending_request && Run_Trigger.Value)
                {
                    ...
                    WebRequestTask = new Task<QArray>(() => GetBookMarks(RA), CTOK);
                    next_web_request = "";
